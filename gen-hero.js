...
<style>
  *{box-sizing:border-box}
  ...
  /* La lámina: hasta diez cartas en una sola imagen, planas y sin sombra
     proyectada, para subirla como referencia a una IA que solo aprovecha
     dos o tres imágenes a la vez. */
  .lamina{display:grid;grid-template-columns:repeat(5,630px);gap:44px;padding:56px;position:relative;z-index:3;
    background:#EFEAE0;border-radius:0}
  .lamina .carta{box-shadow:inset 0 0 0 11px var(--tono), 0 6px 14px rgba(0,0,0,.18)}
...
${dorso("dorso")}
${abanico}
<div class="lamina" id="lamina">${pedidas.slice(0, 10).map(c => cara(c)).join("")}</div>
</body></html>`;
...
(async () => {
  ...
  for (let i = 0; i < pedidas.length; i++) {
    ...
    await pagina.locator("#k" + i).screenshot({ path: path.join(salida, nombre), omitBackground: true });
    hechos.push(nombre);
  }
  await pagina.locator("#dorso").screenshot({ path: path.join(salida, "dorso.png"), omitBackground: true });
  hechos.push("dorso.png");
  if (pedidas.length > 1) {
    await pagina.locator("#lamina").screenshot({ path: path.join(salida, "lamina.png") });
    hechos.push("lamina.png");
  }
