...
/* Cada carta distinta tiene su propia clave: en plagas y remedios la clave
   lleva el color (p_comun_cacao), porque cada color hace algo distinto —solo
   le entra a su cultivo— y lleva su propio texto. Antes el catálogo agrupaba
   por diseño y mostraba una sola plaga común por las cinco, así que las
   cartas de cada color no aparecían. */
const fina = x => A.clavesCarta(x)[0];
const porMazo = {};
R.crearMazo(true, true).forEach(x => {
  const k = fina(x), m = R.mazoDe(x);
...
/* una entrada por carta distinta, en el orden del mazo */
const ejemplares = [];
...
/* Las ilustraciones que existen, para mostrarlas en vez del dibujo en vector.
   Se busca como las busca el juego: primero la de color, después la general. */
const path = require("path");
const DIR_ARTE = path.join(__dirname, "public", "cartas");
const hayArte = new Set(fs.existsSync(DIR_ARTE)
  ? fs.readdirSync(DIR_ARTE).filter(f => f.endsWith(".png")).map(f => f.slice(0, -4)) : []);
const regProv = path.join(DIR_ARTE, "provisionales.json");
const provisionales = new Set(fs.existsSync(regProv) ? Object.keys(JSON.parse(fs.readFileSync(regProv, "utf8"))) : []);
const archivoDe = x => A.clavesCarta(x).find(k => hayArte.has(k)) || null;
/* los archivos que la página necesita publicar a su lado */
...
function tarjeta(e){
  ...
  const arch = archivoDe(carta);
  if (arch) usados.add(arch);
  const dib = arch ? `<img src="cartas/${arch}.png" alt="" loading="lazy">` : A.dibujo(carta, tono);
  ...
  return `<article class="carta" id="${k}">
    ...
    <div class="ficha">
      <header>
        <h3>${esc(nombre)}</h3>
        ${(carta.k==="plaga"||carta.k==="remedio")?`<p class="clase">${esc(R.claseCarta(carta))}</p>`:""}
        <p class="meta">
      ...
      <p class="clave"><code>${k}.png</code>${A.clavesCarta(carta).length > 1
        ? ` <span class="respaldo">si falta, usa <code>${A.clavesCarta(carta)[1]}.png</code></span>` : ""}
        ${arch && provisionales.has(arch) ? `<span class="prov">ilustración provisional</span>` : ""}
        ${arch ? "" : `<span class="prov">aún en vector</span>`}</p>
    </div>
  </article>`;
}
...
const secciones = FAMILIAS.map(f => {
  ...
  return `<section class="familia" id="fam-${f.id}">
    <div class="encabeza">
      <h2>${f.titulo}</h2>
      <p>${f.lead}</p>
      <p class="cuenta">${items.length} cartas distintas · ${items.reduce((a,e)=>a+todo[e.k],0)} en el mazo</p>
    </div>
    <div class="rejilla">${items.map(tarjeta).join("\n")}</div>
  </section>`;
}).join("\n");

const totalDistintas = ejemplares.length;
/* diseños generales: los que hacen falta dibujar como mínimo (29) */
const totalDisenos = new Set(ejemplares.map(e => A.claveCarta(e.carta))).size;
...
.lamina svg,.lamina img{width:100%;height:100%;display:block;object-fit:contain}
.respaldo{font-size:11.5px;color:var(--tinta-suave);margin-left:4px}
.prov{display:inline-block;margin-left:6px;font-size:10.5px;font-weight:700;letter-spacing:.03em;
  color:#8A5A12;background:rgba(217,122,22,.14);border-radius:3px;padding:1px 6px}
...
.clase{margin:-3px 0 7px;font-size:11px;font-weight:700;letter-spacing:.02em;color:var(--tinta-suave,#6b5d4a);text-transform:uppercase}
...
<div class="marco">
  <header class="portada">
    ...
    <div class="cifras">
      <span class="cifra"><b>${totalDistintas}</b>cartas distintas</span>
...
/* La página muestra las ilustraciones desde cartas/, así que al publicarla
   hay que subirlas a su lado. Esta lista dice cuáles. */
fs.writeFileSync("catalogo-imagenes.json", JSON.stringify([...usados].sort(), null, 1));
console.log("catalogo-cosecha.html ·", totalDistintas, "cartas distintas ·", usados.size, "ilustraciones ·", totalCartas, "cartas ·",
