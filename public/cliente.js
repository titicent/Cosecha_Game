...
function sonarSegunVista(){
  ...
  if(primeraVista){                      /* al reconectar no se repite lo ya ocurrido */
    ...
    else if(V.evento.tipo === "clima") { S.efecto("clima"); anunciarClima(V.evento.clima); }
    else S.efecto(V.evento.tipo);
    if(V.evento.tipo !== "clima") animarJugada(V.evento);
  }
  const mio = miTurno();
...
/* ── El clima cae sobre toda la mesa ────────────────────────── */
function anunciarClima(c){
  if(!c) return;
  const capa = document.createElement("div");
  capa.className = "tormenta";
  capa.innerHTML = `<div class="tcaja"><b>${esc(c.nombre)}</b><span>${esc(c.texto)}</span></div>`;
  document.body.appendChild(capa);
  setTimeout(() => { capa.classList.add("ido"); setTimeout(() => capa.remove(), 500); }, 2600);
}
...
function animarJugada(ev){
  ...
  const destino = ev.destino
    ? (document.querySelector(`[data-mata="${ev.destino.j}.${ev.destino.o}"]`) ||
