...
/* Ventana de diagnóstico: solo lectura, lo mismo que el servidor ya te mandó
   a ti. No deja ver cartas ajenas ni permite jugar nada; sirve para mirar el
   estado desde la consola y para que la prueba de navegador compruebe que lo
   pintado concuerda con lo que dicen las reglas. */
window.COSECHA = { get estado() { return V; } };
let recibido = 0, restante = null, tic = null;
try { sesion = JSON.parse(localStorage.getItem("cosecha.sesion") || "null"); } catch(e) {}
const guardar = () => { try { localStorage.setItem("cosecha.sesion", JSON.stringify(sesion)); } catch(e) {} };
...
/* ── Enrutado ───────────────────────────────────────────────── */
/* La hora del cafetal de fondo la decide la pantalla en la que estás. Con
   Espantos en la baraja se hace de noche, que es cuando andan. */
function escena() {
  if (!V) return "portada";
  if (!V.iniciada) return "sala";
  return (V.opciones && V.opciones.espantos) ? "noche" : "mesa";
}

/* Al cambiar de pantalla, lo nuevo entra con un movimiento corto. No es
   adorno: avisa de que cambió el sitio donde estás, que si no el salto entre
   la sala y la mesa se siente como un parpadeo. */
let escenaAnterior = null;
function pintar() {
  $cod.textContent = V && V.codigo ? V.codigo : "";
  $mazo.textContent = (V && V.iniciada && innerWidth > 400) ? "mazo " + V.mazo : "";
  const esc = escena();
  document.body.dataset.escena = esc;
  if (esc !== escenaAnterior) {
    escenaAnterior = esc;
...
/* ── Bienvenida ─────────────────────────────────────────────────
   Tres pantallas que cuentan el juego en medio minuto. Sale sola la primera
   vez y después solo si la piden, porque a la segunda partida estorba. Cada
   una muestra una carta de verdad: se entiende mejor viendo la carta que se
   va a tener en la mano que leyendo un párrafo sobre ella. */
const PASOS = [
  { carta:{k:"cultivo",c:"cafe"}, titulo:"Siembra tu finca",
    texto:"Cada turno tienes <b>dos jornales</b>. Sembrar, plagar, curar o botar cartas "+
          "cuesta uno; una faena cuesta los dos. Con <b>cuatro cultivos distintos y sanos</b> "+
          "cantas cosecha: si tu finca aguanta una vuelta de la mesa, ganas." },
  { carta:{k:"plaga",c:"cacao",t:"comun"}, titulo:"Y cuida la del vecino",
    texto:"Las plagas arruinan una mata; la segunda la acaba. <b>El color manda</b>: "+
          "una plaga de cacao solo entra en cacao, y un remedio de café solo cura café. "+
          "La huerta sirve para todo." },
  { carta:{k:"faena",tr:"chaparron"}, titulo:"El clima no pregunta",
    texto:"Cada tres vueltas cae un clima que le pega a todos por igual. Y cuando la "+
          "tierra se agota, gana quien más <b>puntos de cosecha</b> tenga: uno por mata "+
          "sana, dos por certificada." }
];

function bienvenida(desde) {
  let i = 0;
  const pinta = () => {
    const p = PASOS[i];
    $modal.dataset.abierto = "1";
    $modal.innerHTML = `<div class="telon"><div class="dialogo bienve">
      <div class="bpasos">${PASOS.map((_,k) =>
        `<span class="bpunto ${k===i?"hoy":""}"></span>`).join("")}</div>
      <div class="bcarta">${A.caraCarta(p.carta, R.colorCarta(p.carta), true)}</div>
      <h3>${esc(p.titulo)}</h3>
      <p>${p.texto}</p>
      <div class="fila">
        ${i > 0 ? `<button class="btn claro" data-atras="1">Atrás</button>` : ""}
        <button class="btn oro" data-sigue="1">${i === PASOS.length-1 ? "¡A sembrar!" : "Siguiente"}</button>
      </div>
      ${i === 0 ? `<button class="blink" data-saltar="1">Ya lo conozco</button>` : ""}
    </div></div>`;
    const cerrar = () => {
      try { localStorage.setItem("cosecha.bienvenida", "1"); } catch(e) {}
      cerrarModal();
      /* Cerrar la bienvenida es un clic del jugador, que es justo lo que el
         navegador exige para dejar sonar el audio. Buen momento para empezar. */
      try { S.arrancar(); } catch(e) {}
    };
    const s = $modal.querySelector("[data-sigue]");
    s.onclick = () => { if (i === PASOS.length-1) return cerrar(); i++; pinta(); };
    const a = $modal.querySelector("[data-atras]"); if (a) a.onclick = () => { i--; pinta(); };
    const q = $modal.querySelector("[data-saltar]"); if (q) q.onclick = cerrar;
  };
  pinta();
}
...
/* ── Portada ────────────────────────────────────────────────── */
const MUESTRARIO = [
  {c:{k:"cultivo",c:"cafe"}, t:"Cultivos"},
  {c:{k:"plaga",c:"cacao",t:"comun"}, t:"Plagas"},
  {c:{k:"remedio",c:"platano",t:"casero"}, t:"Remedios"},
  {c:{k:"faena",tr:"trueque"}, t:"Faenas"}
];
const vitrinaHTML = () => MUESTRARIO.map((m,i) =>
  `<div class="vcarta" style="animation-delay:${i*.12}s">
    ${A.caraCarta(m.c, R.colorCarta(m.c), false)}<span class="vetiq">${m.t}</span></div>`).join("");

/* Cuando llegan las ilustraciones se repinta solo la vitrina: repintar la
   portada entera le borraría el nombre a quien ya estaba escribiendo. */
function refrescarVitrina() {
  const v = $app.querySelector(".vitrina");
  if (v) v.innerHTML = vitrinaHTML();
}

function portada() {
  $atril.innerHTML = "";
  $app.innerHTML = `
    <div class="vitrina">${vitrinaHTML()}</div>
    <div class="panel">
      <h2>Arma la mesa</h2>
      <p>De dos a seis finqueros, cada uno en su propio teléfono.</p>
      <input class="campo" id="nombre" maxlength="14" placeholder="Tu nombre"
        value="${esc(localStorage.getItem("cosecha.nombre")||"")}">
      <div style="height:11px"></div>
      <button class="btn ancho oro" id="bCrear">Armar la mesa</button>
    </div>
    <div class="panel">
      <h2>Entrar con código</h2>
      <p>Te lo pasa quien armó la mesa.</p>
  ...
  const nom = () => { const n = document.getElementById("nombre").value.trim();
    localStorage.setItem("cosecha.nombre", n); return n; };
...
function sala() {
  ...
  $app.innerHTML = `
    ...
    <p class="rot" style="text-align:center;margin:2px 0 8px">${V.jugadores.length} de 6 sillas ocupadas${V.jugadores.length<2?" · falta al menos una persona más":""}</p>
    <div class="mesa">${V.jugadores.map((j,i) => `
      <div class="silla ${i===V.yo?"mia":""}">
        ...
        <div class="datos"><div class="linea"><span class="quien">${esc(j.nombre)}</span>
          ${j.bot?`<span class="mini">vecino ${esc(j.bot)}</span>`:""}
    ...
    ${V.anfitrion ? `<div class="panel">
      <h2>Vecinos de la máquina</h2>
      <p>Añade vecinos artificiales para jugar solo o para completar la mesa.</p>
      <div class="seg">
        <button data-bot="novato">+ Vecino novato</button>
        <button data-bot="normal">+ Vecino normal</button>
        <button data-bot="experto">+ Vecino baquiano</button>
        ${V.jugadores.some(j=>j.bot)?`<button data-quitabot="1">− Quitar vecino</button>`:""}
      </div>
      <p class="junto">El novato improvisa, el normal juega bien casi siempre y el baquiano
      siempre elige lo mejor que ve. Ninguno mira tus cartas: deciden con lo que hay en la mesa.</p>
    </div>` : ""}
    <div class="panel">
      <h2>Reglas de la partida</h2>
      <p>Las define quien creó la sala.</p>
      <label class="opt"><input type="checkbox" id="oBon" ${o.bonanza?"checked":""} ${anf?"":"disabled"}>
        <span>Jugar con <b>Bonanza</b>: el vivero bajo techo, plagas resistentes, bioinsumos y
        cuatro faenas más.</span></label>
      <label class="opt"><input type="checkbox" id="oEsp" ${o.espantos?"checked":""} ${anf?"":"disabled"}>
        <span>Jugar con <b>Espantos</b>: el injerto silvestre y los espantos del monte —el Mohán,
        la Patasola, el Duende, la Llorona, el Sombrerón y la Madremonte, que no deja cosechar
        a quien maldice.</span></label>
      <label class="opt"><input type="checkbox" id="oClima" ${o.clima?"checked":""} ${anf?"":"disabled"}>
        <span>Con <b>clima</b>: cada tres vueltas a la mesa cae un aguacero, una sequía o una
        bonanza que golpea a todos por igual.</span></label>
      <label class="opt"><input type="checkbox" id="oCert" ${o.metaCertificada?"checked":""} ${anf?"":"disabled"}>
        <span>Meta <b>certificada</b>: para ganar no basta con tener los cultivos sanos, hay que
        tenerlos certificados. Partidas más largas y difíciles.</span></label>
      <label class="opt"><input type="checkbox" id="oDuelo" ${o.duelo?"checked":""} ${anf?"":"disabled"}>
        <span>Mano a mano: con dos jugadores se gana con cinco cultivos.</span></label>
      <label class="opt"><input type="checkbox" id="oAp" ${o.aprendiz?"checked":""} ${anf?"":"disabled"}>
        <span>Modo aprendiz: el mayordomo sugiere jugadas. Se activa para todos.</span></label>
  ...
  const enviar = () => mandar({t:"opciones", opciones:{
    bonanza: document.getElementById("oBon").checked,
    duelo: document.getElementById("oDuelo").checked,
    metaCertificada: document.getElementById("oCert").checked,
    aprendiz: document.getElementById("oAp").checked,
    espantos: document.getElementById("oEsp").checked,
    clima: document.getElementById("oClima").checked,
    segundosTurno: V.opciones.segundosTurno, minutosJugador: V.opciones.minutosJugador }});
  ["oBon","oDuelo","oAp","oEsp","oCert","oClima"].forEach(id => document.getElementById(id).onchange = enviar);
...
/* Las matas se reparten entre la mesa ($app) y tu propia zona ($atril),
   así que el clic se delega en el documento: cubre los dos. */
document.addEventListener("click", ev => {
  const b = ev.target.closest("[data-mata]");
  if (!b || !V || !V.iniciada) return;
  const [ji, oi] = b.dataset.mata.split(".").map(Number);
  if (!miTurno()) { infoMata(ji, oi); return; }
  if (sel === null) { infoMata(ji, oi); return; }
  tocarMata(ji, oi);
});
function infoMata(ji, oi) {
  const j = V.jugadores[ji], o = j.finca[oi];
  if (!o) return;
  const txt = R.explicaMata(o);
  $modal.dataset.abierto = "1";
  $modal.innerHTML = `<div class="telon"><div class="dialogo">
    <div class="fichagrande">${A.dibujoCultivo(o.carta.c)}</div>
    <h3>${esc(R.CULTIVO[o.carta.c].label)}${ji===V.yo?" (tuyo)":" de "+esc(j.nombre)}</h3>
    <p>${txt}</p>
...
function blancos() {
  ...
  /* El trueque y la Patasola se juegan en dos toques: primero una mata, luego
     la otra. Mientras haya un primer toque hecho, solo se ofrecen los cambios
     que salen de esa mata. */
  if (paso) js = js.filter(j => j.a.j === paso.j && j.a.o === paso.o);
  return js;
}
const esCambio = c => c && c.k === "faena" && (c.tr === "trueque" || c.tr === "patasola");
function esBlanco(ji, oi) {
  return blancos().some(j =>
    (j.o !== undefined && j.j === ji && j.o === oi) ||
    (j.a && (paso ? (j.b.j === ji && j.b.o === oi) : (j.a.j === ji && j.a.o === oi))));
}
function tocarMata(ji, oi) {
  const js = blancos(), carta = V.mano[sel];
  if (esCambio(carta) && !paso) {
    if (!js.some(j => j.a.j === ji && j.a.o === oi)) return;
    paso = {j:ji, o:oi}; return mesa();
  }
  const j = js.find(x =>
    (x.o !== undefined && x.j === ji && x.o === oi) ||
    (x.b && x.b.j === ji && x.b.o === oi));
  if (!j) return;
  if (ji === V.yo && R.dañina(j.tipo)) return confirmarAutogol(j);
...
function fichaHTML(o, ji, oi, chica) {
  const est = R.estadoMata(o), sano = R.estaSano(o), blanco = esBlanco(ji, oi);
  const blind = R.certificado(o);
  const plaga = o.plagas[0], rem = o.remedios[0];
  const sello = plaga ? `<span class="sello">${A.dibujo(plaga, R.CULTIVO[plaga.c].hex, true)}</span>` : "";
  const escudo = est === "certificado" ? `<span class="escudo">${A.dibujo(rem, "#FBFAF7", true)}</span>`
    : est === "protegido" ? `<span class="escudo vac">${A.dibujo(rem, "#FBFAF7", true)}</span>` : "";
  const anillo = (V.metaCertificada && blind) ? `<span class="blindado"></span>` : "";
  return `<button class="ficha ${chica?"mini":""} ${sano?"":"enferma"} ${blanco?"blanco":""}" data-mata="${ji}.${oi}">
    <span class="cara">${A.dibujoCultivo(o.carta.c)}</span>${anillo}${sello}${escudo}</button>`;
}
...
function rivalHTML(i, pos) {
  ...
  const meta = V.metaCertificada ? j.certificados : j.sanos;
  ...
  return `<div class="rival ${pos} ${activo?"activo":""} ${j.fuera?"ido":""} ${j.lista?"lista":""}" data-silla="${i}">
    ...
    <div class="rfinca">${j.finca.map((o,oi)=>fichaHTML(o,i,oi,true)).join("")}</div>
  </div>`;
}
...
function mesa() {
  ...
  const centro = `<div class="pila">
    <div class="monton">${V.descarte?A.caraCarta(V.descarte, R.colorCarta(V.descarte), false):`<div class="vaciopila">COSECHA</div>`}</div>
  ...
  $app.innerHTML = `<div class="tablero">
    ...
    ${fin}
    ${V.clima?`<div class="climachip" title="${esc(V.clima.texto)}">🌦 ${esc(V.clima.nombre)}</div>`:""}
...
function atril() {
  if (V.ganador !== null || V.terminada || (V.jugadores[V.yo] && V.jugadores[V.yo].fuera)) {
    $atril.innerHTML = ""; document.documentElement.style.setProperty("--mio", "0px"); return; }
  const mio = miTurno();
  let panel = "";
  ...
  if (V.pendiente) panel = `<div class="fajita">Hay una respuesta pendiente…</div>`;
    ...
    const js = blancos(), carta = V.mano[sel];
    const sueltas = js.filter(j => j.o === undefined && !j.a);
    let b = `<div class="acciones">`;
    if (paso) b += `<button class="acc tenue" data-volver="1">Elegir otra mata</button>`;
    ...
    if (!js.length) {
      ...
      if (V.jornales >= 1) b += `<button class="acc" data-descartar="1">Botar cartas</button>`;
    } else if (js.some(j => j.o !== undefined || j.a))
      b += `<button class="acc tenue" disabled>${paso ? "Ahora toca la otra mata" :
        esCambio(carta) ? "Toca la primera mata del cambio" : "Toca la mata marcada en la mesa"}</button>`;
    ...
    b += ` · <span class="jorn">${"🪓".repeat(Math.min(V.jornales,4))||"sin jornal"}</span>`;
    b += `</div>`;
    if (V.sugerencias && V.sugerencias.length)
      b += `<div class="sugmini">💡 ${esc(V.sugerencias[0].etiqueta)}</div>`;
    /* Botar cuesta un jornal y se puede en cualquier momento del turno, haya o
       no jugada: justo cuando no la hay es cuando más sirve renovar la mano.
       Cerrar el turno se puede una vez que trabajaste; al empezar, si hay
    ...
    if (!hay) b += `<button class="acc tenue" data-pasar="1">No puedo jugar, pasar</button>`;
    else if (yaTrabajo) b += `<button class="acc tenue" data-pasar="1">Cerrar mi turno</button>`;
    b += `</div>`;
    panel = b;
  }

  const meta = V.metaCertificada ? V.jugadores[V.yo].certificados : V.jugadores[V.yo].sanos;
  const miFinca = V.jugadores[V.yo].finca;
  ...
  $atril.innerHTML = `<div class="zonami">
    ...
    <div class="filami">
      ...
      <div class="ladofinca">${fincaHTML}</div>
    </div>
  </div>`;

  const z = $atril.querySelector(".zonami");
  if (z) document.documentElement.style.setProperty("--mio", Math.ceil(z.offsetHeight) + "px");
...
/* Las reglas permiten plagar tu propia finca, y a veces es la única jugada que
   queda, pero conviene preguntar antes de que sea un resbalón del dedo. */
function confirmarAutogol(j) {
  $modal.dataset.abierto = "1";
  $modal.innerHTML = `<div class="telon"><div class="dialogo">
    <h3>¿Contra tu propia finca?</h3>
    <p>Vas a <b>${esc(j.etiqueta.toLowerCase())}</b>. Es legal —las reglas lo permiten, y a
...
function pintarPendiente() {
  ...
  if (!p.mio) {
    $modal.innerHTML = `<div class="telon"><div class="dialogo">${arte}
      ...
      ${p.tipo==="malla" ? "Se espera la respuesta de quien recibió el ataque." : "Está eligiendo otro objetivo."}</p>
      <div class="cuenta">${seg}</div></div></div>`;
    return;
  }
  if (p.tipo === "malla") {
    if ($modal.dataset.malla === "1") { const c = $modal.querySelector(".cuenta"); if (c) c.textContent = seg; return; }
    $modal.dataset.malla = "1";
    $modal.innerHTML = `<div class="telon"><div class="dialogo">${arte}
      <h3>Te atacaron</h3><p>${esc(p.quien)} juega ${esc(R.nombreCarta(p.carta))} contra ti.
      Puedes gastar tu malla de sombra para que no te toque. No robas después de usarla, así que
      empezarás tu turno con una carta menos.</p>
      <div class="cuenta">${seg}</div>
      <div class="fila"><button class="btn" data-si="1">Usar la malla</button>
      <button class="btn claro" data-nel="1">Aguantar el golpe</button></div></div></div>`;
    $modal.querySelector("[data-si]").onclick = () => { mandar({t:"malla", usar:true}); cerrarModal(); };
    $modal.querySelector("[data-nel]").onclick = () => { mandar({t:"malla", usar:false}); cerrarModal(); };
    return;
  }
...
function cerrarModal(){ $modal.innerHTML = ""; delete $modal.dataset.abierto; delete $modal.dataset.malla; delete $modal.dataset.re; delete $modal.dataset.voto; }
...
document.getElementById("bComo").onclick = () => bienvenida("boton");
...
function sonarSegunVista(){
  ...
  S.ambiente(V.opciones && V.opciones.espantos);
  if(primeraVista){                      /* al reconectar no se repite lo ya ocurrido */
    ...
    /* Un solo anuncio por suceso, en el centro y con el lenguaje de las
       ...
       ni cazar cuál ficha cambió. Jugadas, clima y victoria salen por el
       mismo molde (ver sucesos.js). */
    if(typeof SUCESOS !== "undefined"){
      const ev = V.evento, suyo = ev.ji === V.yo;
      /* El anuncio de cosecha sale después de la jugada que lo provocó. */
      setTimeout(() => anunciarListas(), 0);
      const j = !suyo && ev.ji !== null && ev.ji !== undefined ? V.jugadores[ev.ji] : null;
      /* Botar cartas propias no merece anuncio en toda la mesa: ya lo hiciste
         tú y ya lo viste. El de los demás sí, que ahí no estabas mirando. */
      if(!(suyo && ev.tipo === "descarte")) SUCESOS.suceso(ev, j ? j.nombre : "");
    }
  }
  const mio = miTurno();
...
/* ── Cosecha anunciada ──────────────────────────────────────────
   Cuando una finca pasa a estar lista, la mesa entera tiene que enterarse:
   sale en el centro, con el mismo molde de todo suceso, después de la jugada
   que la completó. Se compara contra lo que había antes para anunciar solo
   el momento en que se completa, no en cada repintado. */
let listasVistas = null;
function anunciarListas() {
  if (!V || !V.jugadores || typeof SUCESOS === "undefined") return;
  const ahora = V.jugadores.map(j => !!j.lista);
  if (listasVistas && listasVistas.length === ahora.length)
    ahora.forEach((l, i) => {
      if (l && !listasVistas[i] && V.ganador === null)
        SUCESOS.suceso({ tipo:"anuncio", ji:i }, i === V.yo ? "" : V.jugadores[i].nombre, { esperar:true });
    });
  listasVistas = ahora;
}
...
function menuPartida(){
  ...
  $modal.innerHTML = `<div class="telon"><div class="dialogo">
    <h3>La partida</h3>
    <p>${V && V.codigo ? "Mesa " + esc(V.codigo) + "." : ""} ${jugando
  ...
  $modal.querySelector("[data-salirya]").onclick = () => {
    ...
    $modal.innerHTML = `<div class="telon"><div class="dialogo">
      ...
      <p>${jugando
        ? "Tus cartas y tus cultivos vuelven al descarte y la partida sigue sin ti. No podrás volver a entrar a esta partida."
...
function salirDeLaSala(){
  mandar({t:"salir"});
  sesion = null;
  try { localStorage.removeItem("cosecha.sesion"); } catch(e) {}
  V = null; sel = null; paso = null; primeraVista = true; ultimoEvento = 0; listasVistas = null;
  clearInterval(tic);
...
/* ── Arranque ───────────────────────────────────────────────────
   Las ilustraciones propias llegan en una lista aparte. Antes se pintaba la
   portada sin esperarla y nadie la volvía a pintar, así que la vitrina salía
   con los dibujos viejos en vector aunque tus imágenes ya estuvieran. Ahora
   la portada y la bienvenida esperan la lista, con un tope de tiempo para
   que una red lenta no deje la pantalla en blanco. */
const cargaArte = fetch("cartas/lista.json")
  .then(r => r.ok ? r.json() : [])
  .then(l => { A.usarExternas(l); return true; })
  .catch(() => false);

conectar();
document.body.dataset.escena = "portada";
portada();                       /* primero, con lo que haya, para no dejar en blanco */

/* Llegue cuando llegue la lista, lo que está en pantalla se pone al día. */
cargaArte.then(ok => {
  if (!ok) return;
  if (V) pintar(); else refrescarVitrina();
});

/* La bienvenida sale sola la primera vez, y espera a las ilustraciones para
   no mostrar las cartas en su versión vieja. */
let vistaYa = "1";
try { vistaYa = localStorage.getItem("cosecha.bienvenida"); } catch(e) {}
if (!vistaYa)
  Promise.race([cargaArte, new Promise(r => setTimeout(r, 1500))])
    .then(() => setTimeout(() => { if (!V) bienvenida(); }, 250));
