...
function infoMata(ji, oi) {
  ...
  if (!o) return;
  const est = R.estadoMata(o);
  const txt = R.esVivero(o) ? "Órgano biónico: siempre cuenta como sano. No lo infecta ningún virus y no admite medicinas."
    : est === "infectado" ? "Infectado con un virus " + (o.plagas[0].t === "evo" ? "mutado: solo lo cura una medicina experimental o una cuarentena." : "básico: lo cura cualquier medicina de su color.")
...
function fichaHTML(o, ji, oi, chica) {
  ...
  const virus = o.plagas[0], med = o.remedios[0];
  const sello = virus ? `<span class="sello">${A.dibujo(virus, R.CULTIVO[virus.c].hex, true)}</span>` : "";
...
function rivalHTML(i, pos) {
  ...
  return `<div class="rival ${pos} ${activo?"activo":""} ${j.fuera?"ido":""}" data-silla="${i}">
    ...
    <div class="rcuerpo">${j.finca.map((o,oi)=>fichaHTML(o,i,oi,true)).join("")}</div>
  </div>`;
}
...
function atril() {
  ...
  const cuerpoMio = V.jugadores[V.yo].finca;
  ...
  const cuerpoHTML = `<div class="micuerpo">
      ...
      ${cuerpoMio.length?cuerpoMio.map((o,oi)=>fichaHTML(o,V.yo,oi,false)).join(""):'<span class="sinorg">siembra para<br>armar tu finca</span>'}
    </div>`;
