...
/* El servidor puede vivir en otro sitio que la página (ver config.js). */
const SERVIDOR = (window.COSECHA_SERVIDOR || "").replace(/\/+$/, "");
const direccionWs = () => SERVIDOR ? SERVIDOR.replace(/^http/, "ws")
  : (location.protocol === "https:" ? "wss" : "ws") + "://" + location.host;

/* Cuando la página vive aparte, el servidor gratuito puede estar dormido.
   Se le pregunta por /salud hasta que responda —la misma pregunta lo
   despierta— y, si tarda, se muestra «Despertando la finca». Si responde
   rápido no se ve nada: la pantalla solo sale pasado casi un segundo. */
async function despertar() {
  if (!SERVIDOR) return;
  let visible = false;
  const aviso = setTimeout(() => { visible = true; MENU.despertando(true); }, 900);
  for (;;) {
    try {
      const r = await fetch(SERVIDOR + "/salud", { cache: "no-store" });
      if (r.ok && (await r.json()).ok) break;
    } catch (e) {}
    await new Promise(r => setTimeout(r, 2500));
  }
  clearTimeout(aviso);
  if (visible) await MENU.despertando(false);
}
function conectar() {
...
/* ── Antes de la mesa ───────────────────────────────────────────
   El ingreso, los modos de juego, la configuración, la transición que
   cuenta el juego y la sala de espera viven en menu.js. */
function portada() { MENU.pintar(); }
function sala() { MENU.sala(); }
...
document.getElementById("bComo").onclick = () => MENU.intro(V ? V.opciones : null, null, true);
...
/* ── Arranque ───────────────────────────────────────────────────
   Las ilustraciones propias llegan en una lista aparte. El menú se pinta de
   una vez con lo que haya y se repinta cuando llega la lista, para que las
   escenas de los modos salgan con tus dibujos y no con los de respaldo. */
const cargaArte = fetch("cartas/lista.json")
  .then(r => r.ok ? r.json() : [])
  .then(l => { A.usarExternas(l); return true; })
  .catch(() => false);

despertar().then(conectar);
document.body.dataset.escena = "portada";
portada();

/* Llegue cuando llegue la lista, lo que está en pantalla se pone al día,
   salvo que alguien esté escribiendo: repintar le borraría lo escrito. */
cargaArte.then(ok => {
  if (!ok) return;
  if (V) return pintar();
