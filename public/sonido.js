...
const estado = { efectos: guardado("cosecha.efectos"), fondo: guardado("cosecha.fondo") };
...
const EFECTOS = {
  ...
  injertar()        { tono({f:120, f2:70, tipo:"sawtooth", dur:.5, vol:.22}); ruido({dur:.4, vol:.2, f:400, f2:120}); },
  plagar()     { tono({f:300, f2:105, tipo:"sawtooth", dur:.32, vol:.24}); ruido({dur:.18, vol:.16, f:1200, f2:300}); },
  arrasar()     { ruido({dur:.36, vol:.4, f:1800, f2:120, tipo:"lowpass"}); tono({f:90, f2:45, tipo:"square", dur:.3, vol:.18, ret:.03}); },
  lavar()  { ruido({dur:.22, vol:.3, f:2400, f2:600}); tono({f:220, f2:140, tipo:"triangle", dur:.2, vol:.16}); },
  curar()        { acorde([523.25, 659.25, 783.99]); },
  proteger()      { tono({f:659.25, tipo:"triangle", dur:.3, vol:.26}); },
  certificar()    { acorde([659.25, 987.77, 1318.5], "sine", .42, .2); ruido({dur:.5, vol:.07, f:5000, q:.6, ret:.05}); },
  ladron()       { ruido({dur:.3, vol:.28, f:300, f2:2600}); tono({f:180, f2:520, tipo:"triangle", dur:.26, vol:.16}); },
  cambiazo()   { ruido({dur:.26, vol:.22, f:700, f2:2200}); tono({f:400, f2:600, tipo:"sine", dur:.22, vol:.16, ret:.1}); },
  plaguero()     { [0,.09,.18].forEach(r => tono({f:340, f2:120, tipo:"sawtooth", dur:.22, vol:.17, ret:r})); },
  aguacero()       { ruido({dur:.42, vol:.34, f:3200, f2:400}); },
  ...
  frasco()   { tono({f:1200, f2:900, tipo:"square", dur:.08, vol:.16}); ruido({dur:.5, vol:.13, f:2200, f2:300, ret:.05}); },
  ...
  /* el clima entra como un trueno lejano con viento */
  clima()        { tono({f:70, f2:38, tipo:"sine", dur:.9, vol:.4});
                   ruido({dur:1.5, vol:.2, f:400, f2:1800, q:.5, ret:.1});
                   ruido({dur:.8, vol:.16, f:2200, f2:500, ret:.55}); }
};
const ALIAS = {
  sembrar:"bajar", injertar:"injertar", plagar:"plagar", arrasar:"arrasar", lavar:"lavar",
  curar:"curar", proteger:"proteger", certificar:"certificar", certificar_ya:"certificar",
  llorona_curar:"curar", llorona_arrasar:"arrasar", llorona_lavar:"lavar",
  llorona_certificar:"certificar", saqueo:"ladron", mohan:"ladron", trueque:"cambiazo",
  patasola:"cambiazo", propagacion:"plaguero", chaparron:"aguacero", lindero:"error",
  sombreron:"cambio", jornalExtra:"horas", consejo:"segunda", duende:"aparicion",
  madremonte:"truco", erradicacion:"frasco", finpartida:"derrota"};
...
/* ── Fondo: pulso de la finca y un colchón grave ──────────── */
...
function alternar(cual) {
  ...
  try { localStorage.setItem("cosecha." + cual, estado[cual] ? "1" : "0"); } catch(e) {}
