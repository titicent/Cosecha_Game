...
/* El artículo de cada cultivo. La huerta y la caña son femeninas; sin esto el
   diario decía «se llevó el huerta» y «plagó el caña». Las contracciones van
   aparte porque «a el» y «de el» se dicen «al» y «del», pero «a la», «de la». */
const FEMENINOS=new Set(["huerta","cana"]);
const art=c=>FEMENINOS.has(c)?"la":"el";
const conArt=c=>art(c)+" "+CULTIVO[c].label.toLowerCase();
const alCult=c=>(FEMENINOS.has(c)?"a la ":"al ")+CULTIVO[c].label.toLowerCase();
const delCult=c=>(FEMENINOS.has(c)?"de la ":"del ")+CULTIVO[c].label.toLowerCase();
/* Cada plaga y cada remedio tiene el nombre de lo que es en el campo
   colombiano: la broca es del café, la sigatoka del plátano. Son los mismos
   nombres del taller de ilustraciones, así la carta, el diario y el catálogo
   dicen lo mismo que el dibujo. */
const NOMBRE_PROPIO={
  comun:     {cafe:"Broca",          platano:"Sigatoka",    cacao:"Monilia",         cana:"Barrenador",    huerta:"Langosta"},
  resistente:{cafe:"Roya",           platano:"Picudo",      cacao:"Escoba de bruja", cana:"Diatraea",      huerta:"Hormiga arriera"},
  casero:    {cafe:"Caldo bordelés", platano:"Ceniza",      cacao:"Poda y sellado",  cana:"Melaza trampa", huerta:"Jabón potásico"},
  bioinsumo: {cafe:"Beauveria",      platano:"Trichoderma", cacao:"Bioabono",        cana:"Trichogramma",  huerta:"Microorganismos"}
};
/* La clase dice qué hace la carta en el juego: «Plaga de café»,
   «Bioinsumo de toda la huerta». Va debajo del nombre propio. */
function claseCarta(x){
  ...
  return esEspanto(x)?"Espanto":"Faena";
}
function nombreCarta(x){
  if(!x)return "—";
  if(x.k==="cultivo")return CULTIVO[x.c].label;
  if(x.k==="plaga"||x.k==="remedio"){
    const p=NOMBRE_PROPIO[x.t]&&NOMBRE_PROPIO[x.t][x.c];
    if(p)return p;
    return (x.k==="plaga"?(x.t==="resistente"?"Plaga resistente ":"Plaga "):(x.t==="bioinsumo"?"Bioinsumo ":"Remedio "))+etColor(x.c);
  }
  return FAENA[x.tr];
}
...
/* Lo que se le explica al jugador cuando toca una mata de la mesa. Vive aquí
   y no en el cliente para que diga lo mismo que hacen las reglas. */
function explicaMata(o){
  if(esVivero(o))return "Vivero: crece bajo techo. Cuenta como certificado y vale dos puntos, "+
    "no le entra ninguna plaga y tampoco admite remedios. Eso sí, te lo pueden cambiar o llevar.";
  if(esInjerto(o))return "Injerto silvestre: cuenta como un cultivo más, pero solo "+
    "lo tocan las cartas de huerta.";
  const e=estadoMata(o);
  if(e==="plagado")return o.plagas[0].t==="resistente"
    ? "Plagada con una plaga resistente: solo la quita un bioinsumo o una erradicación."
    : "Plagada: la cura cualquier remedio de su color, o uno de huerta.";
  if(e==="certificado")return "Certificada: vale dos puntos de cosecha y ya nadie la "+
    "toca. No se puede plagar, ni robar, ni cambiar de finca.";
  if(e==="protegido")return "Protegida: para plagarla hay que quitarle antes el remedio.";
  return "Sana: cuenta para la finca completa y vale un punto, pero todavía se puede plagar.";
}
...
/* Jugadas que le hacen daño al dueño de la mata señalada. El cliente la usa
   para avisar antes de que alguien se dispare en su propia finca. */
const DAÑINAS=["plagar","lavar","arrasar","saqueo","mohan","llorona_arrasar","llorona_lavar"];
const dañina=t=>DAÑINAS.includes(t);
...
function atacados(E,ji,jugada){
  ...
  if(["plagar","lavar","arrasar","saqueo","mohan","llorona_curar","llorona_arrasar",
    ...
    return (dañina(t) && jugada.j !== ji) ? [jugada.j] : [];
  }
...
function aplicar(E, ji, idx, jugada, protegidos){
  ...
  const cDe=(j,o)=>mata(j,o).carta.c;
  const nom=(j,o)=>CULTIVO[mata(j,o).carta.c].label.toLowerCase()+
    (j===ji?"":" de "+E.jugadores[j].nombre);
  ...
  switch(jugada.tipo){
    ...
    case "plagar":
      ...
      reg.push(yo.nombre+" plagó "+art(cDe(jugada.j,jugada.o))+" "+nom(jugada.j,jugada.o));
      break;
    case "lavar":
    case "llorona_lavar":{
      const o=mata(jugada.j,jugada.o);
      E.descarte.push(carta,...o.remedios.splice(0));
      reg.push(yo.nombre+" le lavó el remedio "+(FEMENINOS.has(cDe(jugada.j,jugada.o))?"a la ":"al ")+nom(jugada.j,jugada.o));
      sonido="lavar"; break;}
    case "arrasar":
    case "llorona_arrasar":{
      const o=mata(jugada.j,jugada.o);
      reg.push(yo.nombre+" arrasó "+art(cDe(jugada.j,jugada.o))+" "+nom(jugada.j,jugada.o));
    ...
    case "llorona_curar":{
      ...
      reg.push(yo.nombre+" curó "+art(cDe(jugada.j,jugada.o))+" "+nom(jugada.j,jugada.o));
      pasarMaldicion(jugada.j);
      sonido="curar"; break;}
    case "proteger":
      mata(jugada.j,jugada.o).remedios.push(carta);
      reg.push(yo.nombre+" protegió "+art(cDe(jugada.j,jugada.o))+" "+nom(jugada.j,jugada.o));
      pasarMaldicion(jugada.j);
      break;
    case "certificar":
    case "certificar_ya":
    case "llorona_certificar":
      mata(jugada.j,jugada.o).remedios.push(carta);
      reg.push(yo.nombre+" certificó "+art(cDe(jugada.j,jugada.o))+" "+nom(jugada.j,jugada.o));
      pasarMaldicion(jugada.j);
    ...
    case "mohan":{
      ...
      yo.finca.push(o);
      reg.push(yo.nombre+" se llevó "+conArt(o.carta.c)+" de "+E.jugadores[jugada.j].nombre);
      sonido="saqueo"; break;}
    case "propagacion":{
      E.descarte.push(carta);
      let n=0;
    ...
    case "erradicacion":{
      ...
      reg.push(yo.nombre+" erradicó la plaga "+(FEMENINOS.has(cDe(jugada.j,jugada.o))?"de la ":"del ")+nom(jugada.j,jugada.o));
      break;}
  }
...
/* ── Cosecha anunciada ──────────────────────────────────────────
   Completar la finca no gana en el acto: la deja «lista», a la vista de
   todos, y la mesa tiene una vuelta para frenarla. Si al empezar tu siguiente
   turno la finca sigue completa, cosechas y ganas.

   Antes se ganaba en el mismo instante de completar, y con dos jornales por
   turno alguien podía pasar de dos matas a cuatro sin que nadie lo viera
   venir. Medido con `npm run ritmo`: le pasaba a una de cada tres o cuatro
   partidas de tres o cuatro jugadores. Con el anuncio, ninguna.

   Quien pierde el turno —por el Chaparrón, por ejemplo— no alcanza a empezarlo,
   así que tampoco cobra la cosecha hasta el siguiente. */
const lista=(E,j)=>activo(j)&&!j.maldicion&&logrados(E,j)>=E.objetivo;
function ganador(E){
  return (E.cosecha===undefined||E.cosecha===null)?null:E.cosecha;
}
...
function avanzarTurno(E){
  ...
  /* Empieza un turno: si la finca de quien juega aguantó la vuelta, cosecha. */
  if(lista(E,E.jugadores[E.turno])) E.cosecha=E.turno;
}
...
function puntuar(E,ji,carta,jug){
  ...
  /* Con la cosecha anunciada, un vecino con la finca lista gana cuando le
     vuelva el turno: frenarlo es lo más urgente de la mesa. */
  const hayLista=rivales.some(r=>lista(E,r));
  const urg=hayLista?40:(lider>=E.objetivo-1?22:(lider>=E.objetivo-2?8:0));
  const propio=jug.j===ji;
  const rival=jug.j!==undefined&&!propio&&E.jugadores[jug.j];
  const cerca=rival&&mide(rival)>=E.objetivo-1;
  /* Y quien ya la tiene lista se dedica a blindarla para aguantar la vuelta. */
  const aguantar=lista(E,yo)?30:0;
  ...
  switch(jug.tipo){
    ...
    case "certificar_ya": return propio?(meta?(gano(1)?100:90):80)+urg/2+aguantar:(yo.maldicion?68:8);
    case "certificar":
    case "llorona_certificar": return propio?(meta?(gano(1)?100:88):72)+urg/2+aguantar:(yo.maldicion?66:8);
    case "proteger":      return propio?52+aguantar:(yo.maldicion?64:8);
...
function queHace(x){
  ...
  if(x.k==="cultivo"){
    ...
    return CULTIVO[x.c].label+": siémbralo en tu finca. Con cuatro cultivos distintos y sanos cantas cosecha; si aguanta la vuelta, ganas. No puedes tener dos del mismo.";
  }
  /* Plagas y remedios dicen a qué le entran. Antes decían «una mata» a secas,
     y la regla más importante del juego —el color manda— no aparecía escrita
     en ninguna carta. */
  const aQue=c=>c==="huerta"?"a una mata de cualquier cultivo":"a una mata de "+CULTIVO[c].label.toLowerCase()+" o a la huerta";
  if(x.k==="plaga"){
    const r=x.t==="resistente";
    return "Plaga"+(r?" resistente":"")+": le entra "+aQue(x.c)+" y la arruina; una segunda plaga la acaba. Sobre una mata protegida, le lava el remedio."+
      (r?" Solo la quita un bioinsumo o una erradicación.":"");
  }
  if(x.k==="remedio"){
    const b=x.t==="bioinsumo";
    const aQuien=x.c==="huerta"?"a cualquier mata tuya":"a una mata tuya de "+CULTIVO[x.c].label.toLowerCase()+" o a tu huerta";
    return (b?"Bioinsumo":"Remedio")+": le sirve "+aQuien+
      ". Si está sana la "+(b?"certifica de una":"protege")+
      "; si está protegida la certifica para siempre; si está plagada la cura"+(b?", hasta de plagas resistentes":"")+".";
  }
  const D={
    ...
    mohan_platano:"El Mohán se lleva un plátano o una huerta del vecino, aunque esté certificado.",
