...
const PALABRA = {
  ...
  descarte:"Botó cartas", victoria:"¡Cosecha completa!", finpartida:"Se acabó la partida",
  clima:"Cambió el clima"
};

/* Los sucesos sin carta dicen de dónde sacan su imagen y su color. */
const VERDE = "#4A6338", ORO = "#B8891B";
const SIN_CARTA = {
...
function componer(ev) {
  ...
  if (SIN_CARTA[tipo]) {
    ...
    return { arte: s.arte(), color: s.color, palabra: PALABRA[tipo], largo: tipo === "victoria" };
  }
  if (ev.carta && R) {
...
function suceso(ev, quien) {
  if (!ev) return;
  const m = componer(ev);
  if (!m || !m.palabra) return;

  /* Una jugada que llega mientras se muestra otra la reemplaza: si no, en una
     mesa de seis el centro iría atrasado. Pero el clima y la victoria se leen,
     y lo que llegue mientras tanto espera su turno. */
  if (actual) {
    if (actual.largo) { cola.push([ev, quien]); return; }
