...
(async () => {
  ...
  try {
    ...
    /* ── Armar una mesa contra la máquina ────────────────────── */
    await pagina.fill("input", "Ricardo");
    await pagina.locator("button").filter({ hasText: /armar|crear|nueva/i }).first().click();
    await espera(1200);

    /* Añadir vecinos si el panel los ofrece, y arrancar */
    for (let i = 0; i < 2; i++) {
      const add = pagina.locator("button").filter({ hasText: /vecino|añadir|agregar|bot/i }).first();
      if (await add.count() && await add.isEnabled()) { await add.click(); await espera(250); }
    }
    const arranca = pagina.locator("button").filter({ hasText: /empezar|comenzar|iniciar|arrancar/i }).first();
    if (await arranca.count()) await arranca.click();
    ...
    const SALIDAS = "[data-cerrar], [data-no], [data-nel], [data-si]";
    ...
    let jugadas = 0, vistoEscudo = null, vistoDetalle = null;
    ...
    /* 3. Ningún texto en pantalla puede venir del juego anterior. */
    ...
    /* 4. Sin errores de JavaScript en toda la partida. */
