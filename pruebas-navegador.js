...
(async () => {
  ...
  try {
    ...
    /* La bienvenida sale sola la primera vez, y un navegador de pruebas
       siempre es la primera vez. */
    await espera(900);
    const saltar = pagina.locator("[data-saltar]").first();
    if (await saltar.count()) { await saltar.click({timeout:3000}).catch(()=>{}); await espera(400); }
