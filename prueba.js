/* Levanta el servidor y juega partidas completas con clientes reales
   por websocket: crear sala, unirse, jugar, malla de sombra,
...
class Jugador {
  ...
  reaccionar(){
    ...
    if (V.pendiente && V.pendiente.mio) {
      if (V.pendiente.tipo === "malla") this.env({t:"malla", usar: Math.random() < 0.6});
...
(async () => {
  await espera(700);
  const casos = [
    [2, {bonanza:true, duelo:true, aprendiz:true, espantos:true}, false],
    [3, {bonanza:true, duelo:false, aprendiz:false, clima:true}, true],
    [4, {bonanza:true, duelo:false, aprendiz:true, espantos:true}, false],
    [6, {bonanza:true, duelo:false, aprendiz:false, espantos:true}, true],
    [3, {bonanza:false, duelo:false, aprendiz:false, clima:false}, false],
  ];
  let bien = 0;
  for (const [n, o, cortes] of casos){
    const r = await partida(n, o, cortes);
    if (r.ok){ bien++;
      console.log(`✓ ${n} jugadores${o.espantos?" con Espantos":o.bonanza?" con Bonanza":" solo base"}${cortes?" y con caída de señal":""} → ganó ${r.ganador} tras ${r.turnos} acciones${r.errs.length?" · avisos: "+r.errs.slice(0,2).join("/"):""}`);
