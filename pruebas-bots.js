...
const srv = spawn("node", ["server.js"], { cwd:__dirname, env:{...process.env, PORT:P, PENSAR_BOT:"8"}, stdio:["ignore","ignore","pipe"] });
...
class J{ constructor(){this.V=null;}
  ...
  auto(){const V=this.V;if(!V||!V.iniciada||V.ganador!==null||V.terminada)return;
    if(V.pendiente&&V.pendiente.mio){this.env({t:"malla",usar:Math.random()<.5});return;}
...
(async()=>{
  ...
  for(const conf of [["experto","experto","experto"],["normal","novato"],["experto"]]){
    ...
    yo.env({t:"opciones",opciones:{bonanza:true,espantos:true,duelo:false,aprendiz:false,metaCertificada:false,segundosTurno:0,minutosJugador:0}});
