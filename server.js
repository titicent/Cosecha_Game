...
const servidor = http.createServer((req, res) => {
  ...
  /* /salud: la pregunta «¿estás despierto?». La usan el servicio que visita el
     juego cada diez minutos para que Render no lo duerma, y la pantalla
     «Despertando la finca» cuando la página vive en otro sitio: por eso deja
     que la consulte cualquier origen. Es una respuesta mínima y no guarda nada. */
  if (url === "/salud") {
...
/* Las reglas que el anfitrión puede elegir, siempre validadas aquí: el
   cliente propone y el servidor decide. */
function ponerOpciones(s, o) {
...
function vista(s, yo) {
  ...
  const base = { codigo: s.codigo, iniciada: s.iniciada, yo, anfitrion: yo === 0,
    ...
    jugadores: s.jugadores.map((j,i) => ({ nombre: j.nombre, conectado: j.conectado,
      ...
      /* finca completa, a la espera de aguantar la vuelta para cosechar */
      lista: E ? R.lista(E, E.jugadores[i]) : false })) };
  if (!E) return base;
...
/* ── Ciclo de turno ─────────────────────────────────────────── */
function cantar(E, g) { E.ganador = g; E.evento = { n: ++E.eventoN, tipo:"victoria", ji:g }; E.registro.push("🏆 " + E.jugadores[g].nombre + " cosechó: su finca aguantó la vuelta"); }
...
function ejecutar(s, ji, idx, jugada, protegidos) {
  ...
  consumir(s);
  const carta = E.jugadores[ji].mano[idx];
  const listasAntes = E.jugadores.map(j => R.lista(E, j));
  const r = R.aplicar(E, ji, idx, jugada, protegidos);
  E.registro.push(...r.registro);
  /* El anuncio de cosecha va al diario: es lo que avisa a la mesa de que le
     queda una vuelta para frenarlo. También cuando alguien la pierde. */
  E.jugadores.forEach((j, i) => {
    const ahora = R.lista(E, j);
    if (ahora && !listasAntes[i]) E.registro.push("🌾 " + j.nombre + " tiene la finca lista: si aguanta la vuelta, cosecha");
    else if (!ahora && listasAntes[i]) E.registro.push(j.nombre + " ya no tiene la finca lista");
  });
...
function turnoBot(s) {
  ...
  if (mano.length && E.jornales >= 1) {                   /* sin jugada: bota una (un jornal) y cierra */
  ...
  ws.on("message", datos => {
    ...
    if (m.t === "crear") {
      ...
      /* El menú de modos manda la partida ya configurada: reglas, vecinos de
         ...
         para que no se vea una sala de espera que nadie necesita. */
      if (m.opciones && typeof m.opciones === "object") ponerOpciones(nueva, m.opciones);
    ...
    if (m.t === "descartar") {
      despierta(s, ws.jugador);
      /* Botar cartas cuesta un jornal, no el turno entero: se puede jugar una
         carta y botar con el jornal que queda, o botar primero y seguir. Las
         cartas botadas se reponen al final del turno, como siempre. */
      if (s.E.jornales < 1)
        return error(ws, "No te queda jornal para botar cartas");
